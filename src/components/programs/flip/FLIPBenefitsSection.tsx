...
export function FLIPBenefitsSection() {
  const { data: c } = usePageContent("flip-benefits", {
    ...
    featured_image: flipWomanWhiteboard,
    benefits: defaultBenefits,
  });

  const benefits = c.benefits as typeof defaultBenefits;
  const featuredImage = assetUrl(c.featured_image) || flipWomanWhiteboard;
  ...
  return (
    <section className="py-16 md:py-24 bg-secondary/50">
      <div className="section-container">
        ...
        <StaggerContainer className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 px-4 lg:px-0" staggerDelay={0.08}>
          {benefits.map((benefit, index) => {
            ...
            return (
              ...
              <div className={`card-modern p-5 md:p-6 group h-full flex flex-col relative overflow-hidden ${featured ? "bg-accent border-accent justify-end min-h-64 md:p-8" : ""}`}>
                {featured && (
                  <>
                    <img src={featuredImage} alt="Women developing technology skills together" loading="lazy" className="absolute inset-0 h-full w-full object-cover" />
                    <span className="absolute inset-0 bg-accent/80" aria-hidden="true" />
                  </>
                )}
                <div className={`relative w-12 h-12 md:w-14 md:h-14 rounded-xl flex items-center justify-center mb-4 transition-transform group-hover:scale-110 ${featured ? "bg-accent-foreground/15 text-accent-foreground" : "bg-accent/10 text-accent"}`}>
                  <Icon className="w-6 h-6 md:w-7 md:h-7" />
                </div>
                <h3 className={`relative font-display font-bold mb-2 ${featured ? "text-2xl md:text-3xl text-accent-foreground" : "text-sm md:text-base text-foreground"}`}>
                  {benefit.title}
                </h3>
                <p className={`relative text-xs md:text-sm ${featured ? "text-accent-foreground/90 max-w-md" : "text-muted-foreground"}`}>
                  {benefit.description}
                </p>
              </div>
              </StaggerItem>
