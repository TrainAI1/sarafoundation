        ...
        <div className="section-container">
          ...
          <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-8 max-w-4xl mx-auto">
            {[
              ...
              <div key={tier.title} className="card-modern p-5 md:p-6 text-center">
                <div className="w-12 h-12 md:w-14 md:h-14 mx-auto rounded-xl md:rounded-2xl bg-gradient-to-br from-primary/10 to-accent/10 flex items-center justify-center mb-3 md:mb-4">
                  <Heart className="w-6 h-6 md:w-7 md:h-7 text-primary" />
                </div>
                <h3 className="font-display font-bold text-base md:text-lg text-foreground mb-2">{tier.title}</h3>
                <p className="text-muted-foreground text-xs md:text-sm">{tier.description}</p>
              </div>
            ))}
          </div>
        </div>
