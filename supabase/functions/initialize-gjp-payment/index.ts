...
const AMOUNT_KOBO = 200000; // ₦2,000 admin/processing fee
...
serve(async (req) => {
  ...
  try {
    ...
    const initRes = await fetch(`${PAYSTACK_BASE}/transaction/initialize`, {
      ...
      body: JSON.stringify({
        ...
        metadata: {
          ...
          purpose: "GJP Admin/Processing Fee",
        },
      }),
    });
    ...
    return new Response(
      JSON.stringify({ access_code, reference, amount: AMOUNT_KOBO, currency: "NGN" }),
      { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } },
    );
  } catch (err) {
    console.error("initialize-gjp-payment error:", err);
